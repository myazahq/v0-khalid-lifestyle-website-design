...
import { Download, Users, CheckCircle, XCircle, Mail, X, Send } from "lucide-react";
import { getAllRsvps } from "@/lib/rsvp-services";
import { sendBulkEmailAction } from "@/lib/bulk-email-actions";
import type { Rsvp } from "@/lib/rsvp-types";

export default function AdminAllRsvpsPage() {
	...
	const emailRecipients = filtered.map((r) => ({ email: r.email, firstName: r.firstName }));
	const uniqueRecipients = Array.from(
		new Map(emailRecipients.map((r) => [r.email, r])).values()
	);

	const exportCsv = () => {
		const rows = [
			["Event", "First Name", "Last Name", "Email", "Phone", "Guests", "Table", "Dietary Needs", "Special Requests", "Status", "Cancellation Reason", "Date"],
			...filtered.map((r) => [
				r.eventTitle || "",
				r.firstName,
				r.lastName,
		...
		a.href = url;
		a.download = `all-rsvps-${filter}-${new Date().toISOString().split("T")[0]}.csv`;
		a.click();
			...
			<div className="flex-1 p-8">
				<div className="max-w-6xl mx-auto">
					<div className="flex items-center justify-between mb-8">
						<div>
							<h1 className="text-3xl font-serif font-bold">All Event RSVPs</h1>
							<p className="text-muted-foreground mt-1">RSVPs across all events</p>
						</div>
					...
					<div className="grid gap-4 md:grid-cols-4 mb-8">
						<Card>
							<CardHeader className="pb-2">
								...
								<CardTitle className="text-4xl font-bold text-primary">{rsvps.length}</CardTitle>
							</CardHeader>
						</Card>
						<Card>
							<CardHeader className="pb-2">
								<CardDescription>Confirmed</CardDescription>
								<CardTitle className="text-4xl font-bold text-green-500">{confirmed.length}</CardTitle>
							</CardHeader>
						</Card>
						<Card>
							<CardHeader className="pb-2">
								<CardDescription>Cancelled</CardDescription>
								<CardTitle className="text-4xl font-bold text-red-400">{cancelled.length}</CardTitle>
							</CardHeader>
						</Card>
						<Card>
							<CardHeader className="pb-2">
											...
											{filtered.map((rsvp) => (
												<tr key={rsvp.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
													<td className="py-4 px-4">
														<p className="text-xs font-medium text-primary">{rsvp.eventTitle}</p>
													</td>
													<td className="py-4 px-4">
													...
													<td className="py-4 px-4 text-center font-medium">{rsvp.numberOfGuests}</td>
													<td className="py-4 px-4">
